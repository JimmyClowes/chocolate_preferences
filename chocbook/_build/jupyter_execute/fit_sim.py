...
# 
# Since I have created a `StanModel` class that inherits the underlying modelling capability from `cmdstanpy` (the interface for using Stan in python), I call the `fit` method, which behind the scenes uses [Markov Chain Monte Carlo (MCMC) sampling](https://mc-stan.org/docs/reference-manual/hmc.html) to sample estimates of the parameter values.
...
# 
# This first visualisation shows the known values of the `mu` parameter in the normal distribution of appeal for each chocolate in the simulation (red lines), representing the population average appeal level per chocolate,  superimposed on the spread of paramter values sampled by the model during the model fitting process (blue violins).
# 
